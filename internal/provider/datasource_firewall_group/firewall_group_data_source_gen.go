...
func (t DataType) ValueFromObject(ctx context.Context, in basetypes.ObjectValue) (basetypes.ObjectValuable, diag.Diagnostics) {
	...
	groupMembersAttribute, ok := attributes["group_members"]

	if !ok {
		diags.AddError(
			"Attribute Missing",
			`group_members is missing from object`)

		return nil, diags
	}

	groupMembersVal, ok := groupMembersAttribute.(basetypes.ListValue)

	if !ok {
		diags.AddError(
			"Attribute Wrong Type",
			fmt.Sprintf(`group_members expected to be basetypes.ListValue, was: %T`, groupMembersAttribute))
	}
...
type DataValue struct {
	...
	GroupMembers basetypes.ListValue   `tfsdk:"group_members"`
...
func (v DataValue) ToTerraformValue(ctx context.Context) (tftypes.Value, error) {
	...
	attrTypes["group_members"] = basetypes.ListType{
	...
	case attr.ValueStateKnown:
		...
		val, err = v.GroupMembers.ToTerraformValue(ctx)

		if err != nil {
			return tftypes.NewValue(objectType, tftypes.UnknownValue), err
		}

		vals["group_members"] = val
...
func (v DataValue) ToObjectValue(ctx context.Context) (basetypes.ObjectValue, diag.Diagnostics) {
	...
	var groupMembersVal basetypes.ListValue
	switch {
	case v.GroupMembers.IsUnknown():
		groupMembersVal = types.ListUnknown(types.StringType)
	case v.GroupMembers.IsNull():
		groupMembersVal = types.ListNull(types.StringType)
	default:
		var d diag.Diagnostics
		groupMembersVal, d = types.ListValue(types.StringType, v.GroupMembers.Elements())
		diags.Append(d...)
	}
	...
	if diags.HasError() {
		return types.ObjectUnknown(map[string]attr.Type{
			...
			"group_members": basetypes.ListType{
				ElemType: types.StringType,
			},
...
func (v DataValue) Equal(o attr.Value) bool {
	...
	if !v.GroupMembers.Equal(other.GroupMembers) {
		return false
	}
