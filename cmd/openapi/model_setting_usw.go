...
type SettingUswGetRequest struct {
...
func addSettingUsw() {
	...
	getOp, err := reflector.NewOperationContext(http.MethodGet, "/s/{site}/get/setting/usw")
	if err != nil {
		log.Fatal(err)
	}
	...
	updateOp, err := reflector.NewOperationContext(http.MethodPut, "/s/{site}/set/setting/usw")
	if err != nil {
		log.Fatal(err)
	}
